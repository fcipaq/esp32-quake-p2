...
esp_err_t bsp_audio_i2s_write(uint8_t* data, uint32_t size, size_t* bytes_written) {
    if (!i2s_tx_chan) {
        /* Audio was not initialized before */
        return -1;
    }

    return i2s_channel_write(i2s_tx_chan, data, size, bytes_written, 1000);
}
...
#if !CONFIG_BSP_LCD_TYPE_HDMI
static lv_indev_t *(lv_display_t *disp)
{
